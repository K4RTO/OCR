...
class PytorchPaddleOCR(TextSystem):
    def __init__(self, *args, **kwargs):
        ...
        default_det_path = f"{ocr_models_dir}/{det}"
        default_rec_path = f"{ocr_models_dir}/{rec}"

        # 如果外部已经明确传入了模型路径并存在，则优先使用，不再触发自动下载
        det_model_path = kwargs.get('det_model_path')
        if det_model_path and os.path.exists(det_model_path):
            pass
        else:
            det_model_path = os.path.join(auto_download_and_get_model_root_path(default_det_path), default_det_path)

        rec_model_path = kwargs.get('rec_model_path')
        if rec_model_path and os.path.exists(rec_model_path):
            pass
        else:
            rec_model_path = os.path.join(auto_download_and_get_model_root_path(default_rec_path), default_rec_path)

        rec_char_dict_path = kwargs.get('rec_char_dict_path')
        if not rec_char_dict_path or not os.path.exists(rec_char_dict_path):
            rec_char_dict_path = os.path.join(root_dir, 'pytorchocr', 'utils', 'resources', 'dict', dict_file)
